...
public class AutoDictionary extends ExpandableDictionary {
    ...
    static final int FREQUENCY_FOR_TYPED = 2;
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper {
        DatabaseHelper(Context context) {
            super(context, AutoDictionary.DATABASE_NAME, null, 1);
        }
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            M.w("AutoDictionary: Upgrading database from version " + oldVersion + " to " + newVersion + ", which will destroy all old data");
    ...
    private static class UpdateDbTask extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... v) {
            ...
            for (Entry<String, Integer> entry : this.mMap.entrySet()) {
                ...
                db.delete(AutoDictionary.AUTODICT_TABLE_NAME, "word=? AND locale=?", new String[]{(String) entry.getKey(), this.mLocale});
                if (freq != null) {
                    db.insert(AutoDictionary.AUTODICT_TABLE_NAME, null, getContentValues((String) entry.getKey(), freq.intValue(), this.mLocale));
                }
            }
            return null;
        }
        ...
        private ContentValues getContentValues(String word, int frequency, String locale) {
            ...
            values.put(AutoDictionary.COLUMN_WORD, word);
            values.put(AutoDictionary.COLUMN_FREQUENCY, Integer.valueOf(frequency));
            values.put(AutoDictionary.COLUMN_LOCALE, locale);
            return values;
        }
    }
    ...
    private Cursor query(String selection, String[] selectionArgs) {
        ...
        return qb.query(sOpenHelper.getReadableDatabase(), null, selection, selectionArgs, null, null, DEFAULT_SORT_ORDER);
    }
}
